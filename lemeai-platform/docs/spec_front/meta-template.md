...
4. [PUT `/Atualizar`](#put-apimetatemplateAtualizar)
...
| Campo | Tipo | Obrigatório | Descrição |
|-------|------|-------------|-----------|
| `tipo` | `string` | ✅ | `URL`, `QUICK_REPLY`, `PHONE_NUMBER` ou `COPY_CODE` |
| `texto` | `string` | ❌* | Texto visível no botão. Máx. 25 caracteres. *Não usado para `COPY_CODE`. |
| `url` | `string` | ❌ | Obrigatório para tipo `URL` |
| `telefone` | `string` | ❌ | Obrigatório para tipo `PHONE_NUMBER` (formato E.164, ex: `+5511999999999`) |
| `codigoOferta` | `string` | ❌ | Código de oferta de exemplo. **Obrigatório para tipo `COPY_CODE`** — exibido à Meta para aprovação do template. |

> **`COPY_CODE`:** Exibe ao usuário um botão "Copiar código de oferta" que copia automaticamente o código para a área de transferência. O campo `codigoOferta` é o valor de exemplo enviado à Meta na criação. No disparo da campanha, o código real é passado como parâmetro `coupon_code` (ver [Disparar campanha](campanha.md#9-disparar-campanha)).
...
**Exemplo de request — template com botão de código de oferta (COPY_CODE):**
```json
POST /api/meta/template/Criar
Authorization: Bearer eyJhbGc...
Content-Type: application/json
...
{
  "nome": "cupom_desconto_exclusivo",
  "categoria": "MARKETING",
  "idioma": "pt_BR",
  "textoBody": "Olá, {{1}}! Você ganhou um cupom exclusivo de desconto. Use o código abaixo para aproveitar {{2}} de desconto na sua próxima compra!",
  "textoFooter": "Válido por 7 dias",
  "exemplosBody": ["João", "20%"],
  "botoes": [
    {
      "tipo": "COPY_CODE",
      "codigoOferta": "PROMO20"
    }
  ]
}
```

> O botão `COPY_CODE` exibe automaticamente o texto "Copiar código de oferta" ao usuário. O campo `codigoOferta` é o código de exemplo enviado à Meta para aprovação — no disparo real, o código efetivo é passado via `coupon_code` nos componentes do disparo.
...
## PUT `/api/meta/template/Atualizar`

Edita um template existente diretamente na API da Meta e atualiza o banco local.  
⚠️ **Apenas templates com status `APPROVED` ou `REJECTED` podem ser editados** (restrição da Meta).  
Após a edição, o template retorna ao status `PENDING` e passa por nova análise da Meta.
...
| Campo | Tipo | Obrigatório | Descrição |
|-------|------|-------------|-----------|
| `metaTemplateId` | `int` | ✅ | ID interno do template (`metaTemplateId`) |
| `categoria` | `string` | ❌ | Nova categoria: `MARKETING`, `UTILITY` ou `AUTHENTICATION` |
| `permitirMudancaCategoria` | `bool` | ❌ | Permite que a Meta reclassifique a categoria automaticamente |
| `textoBody` | `string` | ❌ | Novo texto principal. Use `{{1}}`, `{{2}}`... para variáveis. |
| `textoHeader` | `string` | ❌ | Novo texto do cabeçalho (quando `formatoHeader` for `TEXT`) |
| `formatoHeader` | `string` | ❌ | `TEXT`, `IMAGE`, `VIDEO` ou `DOCUMENT` |
| `exemploHeaderHandle` | `string` | ❌ | Handle de exemplo para header de mídia. Obtido via `POST /ObterHandleExemplo`. |
| `exemploHeaderTexto` | `string` | ❌ | Texto de exemplo para header `TEXT` com variável `{{1}}`. |
| `caminhoMidiaHeader` | `string` | ❌ | Caminho local da mídia do header (reutilizado no disparo). |
| `textoFooter` | `string` | ❌ | Novo rodapé |
| `botoes` | `array` | ❌ | Nova lista de botões (substitui os atuais) |
| `exemplosBody` | `array<string>` | ❌ | Novos valores de exemplo para variáveis do body |

> Apenas os campos informados são incluídos no payload enviado à Meta. Se nenhum campo de componente for enviado, apenas `categoria` e/ou `permitirMudancaCategoria` são atualizados.

**Exemplo de request — alterar body e categoria:**
```json
PUT /api/meta/template/Atualizar
Authorization: Bearer eyJhbGc...
Content-Type: application/json

{
  "metaTemplateId": 3,
  "categoria": "UTILITY",
  "textoBody": "Olá, {{1}}! Seu agendamento para {{2}} às {{3}} foi confirmado com sucesso.",
  "exemplosBody": ["Maria", "20/06/2026", "10:00"]
}
```

**Exemplo de request — atualizar header de imagem:**
```json
PUT /api/meta/template/Atualizar
Authorization: Bearer eyJhbGc...
Content-Type: application/json

{
  ...
  "formatoHeader": "IMAGE",
  "exemploHeaderHandle": "4::aW1hZ2UvanBlZw==:ARZGHmFkb3NhZmlhc2RmYXNkZg==",
  "caminhoMidiaHeader": "TemplateMidia/2026-05-29/nova_imagem.jpg",
  "textoBody": "Olá, {{1}}! Confira nossa nova oferta exclusiva!",
  "exemplosBody": ["Pedro"]
}
```
...
{
  "sucesso": true,
  "mensagem": "Template atualizado com sucesso. Aguardando nova aprovação da Meta.",
  "dados": {
    "metaTemplateId": 3,
    "templateMetaId": "1122334455667788",
    "nome": "confirmacao_agendamento",
...
**Response 400 — status inválido para edição:**
```json
{ "sucesso": false, "mensagem": "Apenas templates com status APPROVED ou REJECTED podem ser editados.", "dados": null }
```

**Response 400 — sem conexão ativa:**
```json
{ "sucesso": false, "mensagem": "Nenhuma conexão WhatsApp Meta ativa encontrada para esta empresa.", "dados": null }
```

**Response 400 — erro da API Meta:**
```json
{ "sucesso": false, "mensagem": "Erro na API da Meta: (#100) ...", "dados": null }
```
...
#### MARKETING: Código de oferta (COPY_CODE)

```json
{
  "nome": "oferta_black_friday",
  "categoria": "MARKETING",
  "idioma": "pt_BR",
  "formatoHeader": "TEXT",
  "textoHeader": "🔥 Oferta exclusiva para você!",
  "textoBody": "Olá, {{1}}! Use o código abaixo e ganhe {{2}} de desconto em toda a loja. Oferta válida até {{3}}.",
  "textoFooter": "Para não receber ofertas, responda SAIR",
  "exemplosBody": ["João", "30%", "30/11/2026"],
  "botoes": [
    {
      "tipo": "COPY_CODE",
      "codigoOferta": "BLACK30"
    },
    {
      "tipo": "URL",
      "texto": "Ver ofertas",
...
> **Como disparar este template:** veja o exemplo de disparo com `COPY_CODE` em [campanha.md — Disparar campanha](campanha.md#9-disparar-campanha). O código real de cada disparo é passado como `coupon_code` nos componentes.
